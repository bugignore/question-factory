...
7. **Keyword density — ideal range is 1%–2% (mentions ÷ total words), so the mention count MUST scale with actual document length, not a fixed number.** As a feel for the range: on a 1,000-word page that's roughly 10–20 mentions of the primary keyphrase (full + partial combinations). A recurring failure: reusing a flat "12–16 mentions" target regardless of word count silently produces a very low density once the document is long (e.g. 10 mentions in a 6,500-word document ≈ 0.15% — well under the 1% floor). Fix this with arithmetic, not a memorized number:
   ...
   - **W counts reader-visible prose only — never markup.** The body is pure HTML (per B0), so when computing W, mentally strip every tag AND everything inside its angle brackets (tag names, `style="..."` CSS, `class`, `id`, other attributes, HTML comments) before counting — count only the words a reader would actually see rendered on the page. CSS property names/values (`background`, `border-radius`, `12px`, hex colors, etc.) and raw tag text are NOT words and must never be included in W or in the keyphrase-mention count. This matches exactly how the publishing app's own word counter works (it strips `<...>` including attributes before counting), so your self-reported W should match what the app displays in Step 3.
