...
Notes Factory's v13 prompt/schema instead of Long Post's. Everything runs
from the terminal except the DeepSeek exchange itself, which needs a real
logged-in browser session (DeepSeek has no API key issued here). Chrome is
launched ONCE for the whole run; each topic opens exactly one new tab to
chat.deepseek.com and closes it before the next topic opens its own - never
more than one DeepSeek tab open at a time. The browser runs headed
(headless=False), so this is watchable, not a black box.

Prompt-building, reply-parsing, and hard-fail validation all run in Node
(Automation/core/src/build-automation-prompt-notes.mjs,
Automation/core/src/validate-bundle-notes.mjs) - this script only
orchestrates: CSV -> prompt -> browser -> reply -> Node validator -> file ->
git. The Node validator (validate-bundle-notes.mjs -> validators-notes.mjs)
is GENERATED from notes-factory/index.html's own "Run validation" checks
(see Automation/core/src/gen-validators-notes.mjs) - not a separately
hand-maintained approximation - so a topic that fails here would also fail
in the manual browser tool, and vice versa. Only a HARD FAIL blocks a
topic; anything softer still publishes with a note in Publisher Notes.

--engine deepseek (default) is the only automated browser engine currently
implemented. ChatGPT (chatgpt.com) support was requested but not built here:
its DOM/selectors haven't been reverse-engineered the way DeepSeek's were
(textarea placeholder, send-button SVG signature, per-message Copy-button
class), and guessing at unverified selectors would silently hang or
misbehave rather than fail loudly. --manual mode already works with
ChatGPT (or any chat UI) today via the clipboard, since it never touches
the page's DOM at all - use that until/unless someone wires up real
ChatGPT selectors and extends run_topic_through_chatgpt() below.

A --manual mode also exists (build the prompt, you paste it into
DeepSeek/ChatGPT/Claude yourself and copy the reply back) for when you
don't have the DeepSeek Chrome profile handy, or want to use a different
AI - it is NOT the default; the default is the automated DeepSeek browser
flow above.
...
Usage:
    ...
    python run_pipeline.py --manual --limit 5    # you paste into DeepSeek/ChatGPT/Claude yourself
"""
...
PENDING_DIR = REPO_ROOT / "pending-notes"
PUBLISHED_DIR = REPO_ROOT / "published-notes"
...
YEAR = 2026
# Same mapping as long-post-runner — the CSV's Hindi subject label maps to
# notes-factory's Subject dropdown value + a default Hindi/English split.
SUBJECT_MAP = {
    "हिंदी": ("Hindi", 90),
    "अंग्रेजी": ("English", 60),
    "गणित": ("Maths", 70),
}
DEFAULT_SUBJECT = ("Auto-detect", 70)
...
def load_existing_topics():
    ...
    Notes Factory has no published-notes/index.json (unlike Long Post) — a
    slug is "already published" simply if published-notes/<slug>.json exists
    on disk, checked directly in is_duplicate() instead of via an index."""
    published_slugs = {p.stem for p in PUBLISHED_DIR.glob("*.json")} if PUBLISHED_DIR.exists() else set()
...
def build_prompt(topic, exam, subject, hindi_pct) -> str:
    return run_node("build-automation-prompt-notes.mjs", [
...
def validate_and_bundle(raw_reply, topic, exam, subject, hindi_pct) -> dict:
    out = run_node("validate-bundle-notes.mjs", [
        "--topic", topic, "--exam", exam, "--subject", subject,
        "--hindi", str(hindi_pct), "--year", str(YEAR),
...
def sanity_test():
    ...
    seo = {
        "focusKeyword": "Sanity Test BPSC Notes 2026", "seoTitle": "Sanity Test BPSC Notes 2026",
        "slug": "sanity-test-bpsc-notes-2026",
        "metaDescription": ("Sanity Test BPSC Notes 2026 with easy examples and practice "
                             "questions for exam preparation season padding text.").ljust(150, "."),
        "h1": "हिंदी परीक्षण शीर्षक यहाँ है", "imageAltText": "Sanity Test BPSC Notes 2026 overview", "imagePrompt": "n/a",
    }
    kw = seo["focusKeyword"]
    links = " ".join(f'<a href="https://examnotespdf.in/free-notes/x{i}/">internal {i}</a>' for i in range(5))
    links += ' <a href="https://ncert.nic.in/textbook.php">NCERT</a> <a href="https://en.wikipedia.org/wiki/Hindi">Wiki</a>'
    filler = "यह एक सैनिटी-टेस्ट वाक्य है। "
    ad = ('<div style="background:#f8fafc;border:1px dashed #cbd5e1;border-radius:12px;'
          'padding:10px 14px;margin:18px 0;"><ins class="adsbygoogle" id="ad-slot-{n}"></ins></div>')
    body = (
        f'<img src="https://via.placeholder.com/700x350?text=Diagram" alt="{kw} concept overview">\n'
        f'<h2 id="a">{kw} basics</h2>\n<p>{kw} — {filler * 8} {links}</p>\n{ad.format(n=1)}\n'
        f'<h2 id="b">Doosra hissa</h2>\n<p>{filler * 8}</p>\n{ad.format(n=2)}\n'
        f'<h2 id="c">FAQ</h2>\n<p>{filler * 8}'
        '<sup id="cite-1"><a href="#ref-1">[1]</a></sup><sup id="cite-2"><a href="#ref-2">[2]</a></sup>'
        '<sup id="cite-3"><a href="#ref-3">[3]</a></sup><sup id="cite-4"><a href="#ref-4">[4]</a></sup></p>\n'
        '<ol><li id="ref-1">Ref one</li><li id="ref-2">Ref two</li><li id="ref-3">Ref three</li><li id="ref-4">Ref four</li></ol>\n'
        f'{ad.format(n=3)}'
    )
    raw = (
    ...
    result = validate_and_bundle(raw, "Sanity Test", "BPSC TRE 4", "Hindi", 90)
    print(json.dumps({k: v for k, v in result.items() if k != "bundle"}, indent=2, ensure_ascii=False))

    if not result["pass"]:
        print("\nSANITY TEST FAILED — the fixture itself should always pass. Check validate-bundle-notes.mjs / validators-notes.mjs.")
        sys.exit(1)
    ...
    print(f"\nWrote {out_path} (delete this — it's a fixture, not a real note).")
...
def main():
    ...
    ap.add_argument("--manual", action="store_true",
                     help="Paste into DeepSeek/ChatGPT/Claude yourself instead of the automated Chrome/DeepSeek flow (default is automated). Works with ChatGPT today via the clipboard.")
    args = ap.parse_args()
...
def run_topics_manual(topics, published_slugs, pending_pairs):
    ...
    tab-opening: you're in the loop for the actual AI exchange (DeepSeek,
    ChatGPT, Claude, whichever you paste into), so you see every reply as it
    happens instead of a batch racing ahead unattended."""
    ...
    while i < len(topics):
        ...
        while True:  # retry loop for this one topic — 'r' below re-enters it
            ...
            print("  -> Paste it into DeepSeek, ChatGPT (chatgpt.com), or Claude now, wait for the full reply, then copy that reply.")
